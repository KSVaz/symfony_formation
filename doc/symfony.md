...
Quelques outils :
 ...
 - recherche avec `ctrl + f` -> chercher des propriétés puis `echap` pour quitter la recherche
...
## ENTITY (ENTITÉ) 
- Un table sera représentée par une classe dans Symfony. Les propriétés vont représenter les champs de la table. Seules les propriétés qui auront été marquées par des annotations seront converties dans la base de données en champs.
